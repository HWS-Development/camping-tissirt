...
export default function StayOptions() {
  ...
  const stayOptions = t('staySection.items', { returnObjects: true })
  const translatedStayOptions = Array.isArray(stayOptions)
    ? stayOptions
    : [
        {
          title: 'Tent pitches',
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="rounded-[2.4rem] border border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.95),rgba(240,249,255,0.95))] p-8 shadow-[0_30px_80px_rgba(14,165,233,0.08)] sm:p-10">
          <div className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
            ...
            <p className="text-sm font-semibold uppercase tracking-[0.28em] text-sky-600">{t('staySection.eyebrow')}</p>
            <h2 className="mt-4 text-4xl font-bold text-slate-950">{t('staySection.title')}</h2>
            ...
            <div className="grid gap-5 md:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
              {translatedStayOptions.map((option) => (
